/**
 * Regenerates src/kitchenData.ts from the upstream Emoji Kitchen metadata.
 *
 ...
 * tables, which gzips to roughly 346 KB.
 *
...
async function loadMetadata(): Promise<Upstream> {
    if (existsSync(CACHE)) {
        console.log(`using cached metadata at ${CACHE}`);
        return JSON.parse(readFileSync(CACHE, "utf8")) as Upstream;
    }
    ...
    return JSON.parse(text) as Upstream;
}
