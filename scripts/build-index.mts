...
/**
 * Regenerates kitchenIndex.b64 and kitchenIndexMeta.ts from upstream metadata.
 *
...
const OUT_INDEX = "kitchenIndex.b64";
const OUT_META = "kitchenIndexMeta.ts";
...
const indexSha256 = createHash("sha256").update(b64).digest("hex");

// The payload is a data file rather than a module on purpose. Compiled into a
// plugin it would be the largest file in Vencord by an order of magnitude and
// every user would download it; fetched, only people who open the picker pay.
writeFileSync(OUT_INDEX, b64 + "\n");

writeFileSync(OUT_META,
    `/*
 * Vencord, a Discord client mod
 * Copyright (c) 2026 Giftedx
 * SPDX-License-Identifier: GPL-3.0-or-later
...
//
// Provenance for ${OUT_INDEX}, which holds the payload itself.
//
// Source:    ${SOURCE}
...
/** Digest of ${OUT_INDEX}, checked after fetching so a bad payload cannot load. */
export const INDEX_SHA256 = "${indexSha256}";
`);
...
console.log(`wrote ${OUT_INDEX} and ${OUT_META}`);
console.log(`index sha256=${indexSha256}`);
console.log("remember to repin INDEX_PIN in loadKitchen.ts once the new index is committed");
